...
connection %>% 
  ...
  geom_bar() + facet_wrap(~SEX) +labs(fill = "Day of Week")  +xlab('hours') 
  +ylab('Number of accidents') +ggtitle('Accidents by week,hour,and sex')
...
```{r}
ggplot(final, aes(x=long, y= lat, finll = numAcc)) + geom_polygon(aes(group = group)) +xlab("Longitude") +ylab("Latitude") + labs(fill="Number of Accidents")
```
